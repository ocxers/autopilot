...
**No shortcuts because Codex will review later.** The Codex review loop (Step 4) reviews CODE DIFFS — it cannot verify that a page renders, that a button works, or that there are no console errors. Browser verification (§2.7), runtime verification (§2.8), and the full verification matrix (§2.6) are YOUR responsibility during coding, exactly as in standalone `/autopilot`. "Codex will catch it" is not a valid reason to skip any verification step.
...
### 2.7 Real Browser Verification — MANDATORY for web apps
...
Mark every requirement line **in the run log (§2.10)** — this accounting is *recorded* there, not recited in the chat (see **Reporting to the user** below for what the user actually sees):
...
The bar for finishing the coding phase is unchanged by the reporting rules — those govern only how much of this reaches the chat.
...
1. Generate a code review prompt by following the `/make-review-prompt` **Template B** (code change review) logic inline — do not invoke `/make-review-prompt` as a slash command:
   ...
   - Follow Template B's rules of engagement and output format exactly (adversarial framing, `file:line` evidence requirement, no praise and no per-item "verified OK" list, BLOCKER/MAJOR/MINOR/QUESTION output grouping, explicit verdict, and the terse write-up rules — a few lines per finding, minimum evidence, empty severity groups omitted, and one closing sentence naming what was examined and found sound, which is coverage information, not praise).
...
3. **If `status == REVIEW_DONE`** (Codex has written feedback — it is Claude Code's turn):
   ...
   - **Act on findings:**
     ...
     - After all fixes, open the next round. **This is a "Fix Summary", NOT a "Review Request."** "Review Request" is exclusively a Round 1 term (Step 3) — never reuse it after Round 1. Do not re-generate a review prompt, re-list the findings, describe what you changed, or tell Codex how to review. Codex already has the Round 1 review prompt and knows to read the diff. Append **EXACTLY** the following to `channel.md` (substitute only `<current + 1>` and `<timestamp>`; the "End of Round" marker for the current round was already written by Codex after its Review Feedback — do not duplicate it):
       ...
       🚨 **VERBATIM — DO NOT ADD ANYTHING.** The template above is the COMPLETE output — every character. The `## Round N` heading has NO subtitle (not `## Round N — Review Request`, not `## Round N — Fix Summary`). The block header is `Fix Summary`, NOT `Review Request`. The body is exactly one line: `Fixed, please re-review.` Do NOT add:
       ...
       - ❌ File lists, modified-file summaries, or "re-review scope" sections
       - ❌ Review instructions or attack vectors (Codex already has these from Round 1)
...
3. Act based on the status:
   ...
   - If status is `REVIEW_REQUESTED` or `FIX_COMPLETED`:
     ...
     e. Write your findings in the channel file using the block format above.
        Keep the write-up SHORT — review deeply, report tersely. No preamble,
        no restating the change, no account of how you reviewed. Write, in
        this order:
        - Open: CODEX — Review Feedback — <timestamp>
        - Group findings as BLOCKER / MAJOR / MINOR / QUESTION, worst first
        - Each finding, at most ~4 lines: one-line title; file:line (quote
          <=3 lines of source, and only when the reference alone doesn't show
          the problem); the failure in one sentence (input/state -> wrong
          result); the fix in one sentence
        - Omit empty severity groups — no "none found" sections
        - Everything you checked and found sound: ONE closing sentence naming
          the areas. No per-item "verified OK" list, no pasted code for what
          passes
...
- No praise and no per-item "verified OK" list. The single coverage sentence
  required in step 3.e is not praise — it states what was examined, not what
  was good.
- Be thorough but fair — only flag real issues, not style preferences.
- Review THOROUGHLY, report TERSELY. Investigate as deeply as you need; each
  finding is a few lines, not an essay. Quote the minimum evidence that proves
  the point — never paste whole functions, files, or diffs.
- A round where you found nothing serious is a few lines long, not a page.
```
...
2. Run `/autopilot-eval` **silently** — this grades the whole run (coding + review loop), not just the coding phase. Do this **before** writing the chat report (step 3), so that anything it turns up is already handled by the time the user reads the summary: if it finds ✅ items that do not reproduce, fix them, update the audit log from step 1, and only then write the report (CONDITION (p)). **"Silently" means ZERO eval output in the chat session.** Do NOT print, summarize, or paraphrase any part of the eval results to the user. Specifically:
   - ❌ Score tables, dimension breakdowns, or rating summaries
   - ❌ "Key findings" or "关键发现" sections from the eval
   - ❌ Paraphrased eval conclusions ("overall 3.5", "done score is low", etc.)
   - ❌ ANY text that reveals what `/autopilot-eval` scored or found
   The eval appends one row to `~/.claude/autopilot-eval.md` — that file IS the output. The user reads it there, not in the chat. When following `/autopilot-eval`'s instructions, **skip its Step 6 (Report back)** — that step is for standalone use only; this caller context overrides it. If `/autopilot-eval` is unavailable, skip silently; do not fabricate a scorecard. **Tripwire:** if you are about to type a score, a table, or the word "eval" in your chat output after running it, STOP — you are violating this rule.
3. Report the outcome to the user in the chat **following the "Reporting to the user" section below** — one short fenced code block: the termination reason, one line per requirement group on what now works, a sentence each for anything not done, and a pointer to both `channel.md` (full transcript) and `summary.md` (audit log). If the outcome is `MAX_ROUNDS`, `TIMEOUT`, or `ABORTED`, name the remaining unresolved findings (one line each) so the user knows exactly what's outstanding. Do not reproduce the round-by-round transcript, the per-line accounting, or Codex's findings verbatim — they are already in those two files.


---

## Reporting to the user — short, and in one code block

The full accounting lives in the run log (§2.10) and the audit log (`.autopilot/reviews/<session-id>/summary.md`). What reaches the chat is a SHORT summary inside ONE fenced code block (4 backticks, so nested ``` survive), containing this and nothing beyond it:

- the termination reason (`APPROVED` / `MAX_ROUNDS` / `TIMEOUT` / `ABORTED`)
- one line per requirement group: what now works
- anything NOT done, and why (blockers, `[CODE]`-only items, unresolved Codex findings) — a sentence each
- anything the user must decide or do next
- pointers to `channel.md` (full transcript) and `summary.md` (audit log)

Do NOT put in chat unless asked: command transcripts, test pass/fail counts, pre-existing failures, screenshot paths, the surface map, per-file diffs, the round-by-round review transcript, or a restatement of what each fix did. Point at the run log and `channel.md` instead.
