...
3. Run `/autopilot-eval` **silently** as the final grading step — this grades the whole run (coding + review loop), not just the coding phase. **"Silently" means ZERO eval output in the chat session.** Do NOT print, summarize, or paraphrase any part of the eval results to the user. Specifically:
