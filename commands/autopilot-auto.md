...
2. **Safety checks — run these FIRST, every poll, before anything else:**
   ...
   - If `status` is `APPROVED`, `ABORTED`, `MAX_ROUNDS`, or `TIMEOUT` → go to Step 6. (`APPROVED` is always Codex-written; `MAX_ROUNDS`/`TIMEOUT` are Claude Code's; `ABORTED` can be either side or user-triggered.)
...
3. Act based on the status:
   ...
   - If status is `REVIEW_REQUESTED` or `FIX_COMPLETED`:
     ...
     f. Update the YAML frontmatter:
        ...
        Claude Code never approves — it only fixes and resubmits.
