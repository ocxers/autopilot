...
1. Read `channel.md` and parse the YAML frontmatter: `status`, `round`, `started_at`, `max_rounds`, `timeout_hours`. **If the channel file cannot be read or its YAML frontmatter fails to parse** (missing/malformed delimiters, invalid YAML, missing required fields) — this is likely a partial write by Codex, not permanent corruption. Print a warning in the chat session: "⚠️ Channel file parse error — retrying (possible partial write by Codex)." Wait 1 minute and retry. If the same parse error persists for **3 consecutive retries**, print a persistent warning: "🚨 Channel file appears corrupted after 3 retries. Waiting for TIMEOUT or manual intervention." Continue polling — do NOT set ABORTED. TIMEOUT will eventually terminate the loop if the file is truly unrecoverable. The user can also manually set `status: ABORTED` in the file.
...
3. **If `status == REVIEW_DONE`** (Codex has written feedback — it is Claude Code's turn):
   ...
   - **Act on findings:**
     - Fix each BLOCKER and MAJOR finding. Apply the same implementation discipline as the coding phase (§2.5–2.9): small batches, run tests/typecheck, browser verification if UI changes, adversarial self-review of the fix itself. MINOR/QUESTION items do not require code changes — acknowledge or respond in the fix summary.
