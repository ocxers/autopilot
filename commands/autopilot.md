...
As the **last action** before finishing, invoke `/autopilot-eval` to score this run and append one row to `~/.claude/autopilot-eval.md`. This is automatic — the user does not trigger it manually.

- **The grader must be independent, not you.** `/autopilot-eval` spawns a fresh judge subagent and re-verifies your ✅ claims hands-on (re-runs tests / re-drives the browser). Do **not** grade from your own `summary.md` — that inherits any lie you made. Let the eval do its job; report its verdict honestly, false-positives first.
